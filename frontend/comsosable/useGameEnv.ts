...
import { GridWorldGameScene } from "~/utils/GameScenes";
...
export default function useGameSceneFactory(
  ...
  switch (name) {
    ...
    case "GridWorld":
      const gridWorldScene: GridWorldGameScene = new GridWorldGameScene(
        <Envs.GridWorldEnv> env,
      );
      return createGridWorldScene(
        gridWorldScene,
        <Envs.GridWorldEnv> env,
        parent,
      );
...
function createTaxiScene(
  ...
  const config: Phaser.Types.Core.GameConfig = {
    type: Phaser.AUTO,
    parent: parent,
    width: Games.Taxi.TaxiGlobals.tileWidth *
      11 *
      Games.Taxi.TaxiGlobals.scale,
    height: Games.Taxi.TaxiGlobals.tileHeight *
      7 *
      Games.Taxi.TaxiGlobals.scale,
    scale: {
      // Fit to window
...
function createGridWorldScene(
  scene: GridWorldGameScene,
  env: Envs.GridWorldEnv,
  parent: HTMLElement,
): SceneInfo {
  const config: Phaser.Types.Core.GameConfig = {
    type: Phaser.AUTO,
